...
/*
 ...
 | //PD10 <-> FSMC_D15 | /PE11 <-> FSMC_D8   | PF12 <-> FSMC_A6 |------------------+ 
...
*/
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_11 |
...
/*-- FSMC Configuration ------------------------------------------------------*/
  ...
  //上电测试
  //第一次下载程序时需要先写入测试数据
  //SRAM_WriteBuffer((uint8_t*)&TestData, NVRAMTEST_ADDR, 4);
  SRAM_ReadBuffer((uint8_t*)&TempU32, NVRAMTEST_ADDR, 4);
...
void SRAM_WriteBuffer(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumHalfwordToWrite)
{
  for (; NumHalfwordToWrite != 0; NumHalfwordToWrite--) /* while there is data to write */
  {
    ...
    *(uint8_t *) (NVRAM_ADDR + WriteAddr) = *pBuffer++;
...
void SRAM_ReadBuffer(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumHalfwordToRead)
{
  for (; NumHalfwordToRead != 0; NumHalfwordToRead--) /* while there is data to read */
  {
    ...
    *pBuffer++ = *(__IO uint8_t*) (NVRAM_ADDR + ReadAddr);
